...
| **1** | **Baryon Acoustic Oscillations (BAO)** | **DESI 2024 DR1 & SDSS BOSS DR12** | Sound horizon propagation via 3D $\xi(r)$ | Idealized First-Principles Benchmark | Spatial correlation structure within the $100\ h^{-1}\mathrm{Mpc}$ box comoving frame |
...
* **[Data Provenance Manifest](data/PROVENANCE.md)** & **[Third-Party Licenses](data/DATA_LICENSES.md)**
...
$$\kappa_0 = \frac{\hbar^2 G^2}{c^7 k_B} \approx 1.6487 \times 10^{-125}\ \mathrm{m}\cdot\mathrm{s}^3\cdot\mathrm{K}\cdot\mathrm{kg}^{-1}$$

### 2. Global Conformal Boundary Transition (Roger Penrose CCC)
Activates when cosmic expansion reaches asymptotic dilution ($a \ge 7.00$, $\rho / \rho_0 = 1/343 \approx 0.29\%$), where the future spacelike hypersurface $\mathscr{I}^+$ conformally rescales into the hot Big Bang past hypersurface $\mathscr{I}^-$.
...
### 3. Ultra-Fast Headless Production Simulations (CLI)
For batch multi-eon production runs and large lattice sizes ($64^3, 128^3, 256^3$) at maximum throughput ($> 500\text{ steps/s}$):
```bash
# Headless multi-eon run on GPU (64³ resolution, 500 Mpc box)
python scripts/run_headless_simulation.py --gpu --grid 64 --box 500 --eons 2

# Fast CPU headless run
python scripts/run_headless_simulation.py --cpu --grid 32 --box 500 --eons 1
```

### 4. Interactive 24/7 Cosmological Server & Web Dashboard
```bash
# Standard Resolution (32x32x32) on CPU
...
# 3. SPARC 2020 Cusp-Core Problem & Galaxy Rotation Curves (175 Galaxies)
python -m observational.compare_halo --all-galaxies

# 4. Pantheon+ (2022) Supernovae & Hubble Tension Resolution (1,701 SNe Ia)
...
│   ├── halo_data.py               # SPARC 2020 galaxy catalog (175 galaxies) & NFW/Burkert halo models
│   ├── halo_analyzer.py           # Density profiles rho(r), slopes gamma(r), and rotation curves V_c(r)
│   ├── compare_halo.py            # Cusp-Core CLI & publication figure generator (supports --all-galaxies)
...
│   ├── pantheon_2022/             # pantheon_plus_full_1701.json, pantheon_plus_supernovae.json
│   └── nanograv_2023/             # nanograv_15yr_pulsars.json
├── legacy/                        # Modelos y prototipos 2D históricos (simulador_cosmologico.py, simulador_reotransductor.py)
├── scripts/                       # Production utilities (run_headless_simulation.py, run_convergence_study.py)
├── docs/
