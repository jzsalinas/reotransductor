"""
Simulador Cosmológico 3D Interactivo (CPU / NumPy)
Visualizador de escritorio con Matplotlib que consume el motor central CosmologicalEngine.
"""
...
# Inicializar motor central (Única Fuente de Verdad)
GRID_SIZE = 32
engine = CosmologicalEngine(grid_size=GRID_SIZE, use_gpu=False, auto_resume=True, initial_speed=20)
...
ax_info.axis('off')
ax_info.text(0.5, 0.98, "TELEMETRÍA 3D [CPU / NumPy]", fontweight='bold', fontsize=9.5, ha='center', va='top', color='#0f172a')
...
def animate_3d(frame):
    # 1. Integración en el motor central
    engine.step_batch()
    ...
    fig.suptitle(f"Simulación Cosmológica 3D [CPU]: Reotransductor, CMB & Bekenstein | Eón N = {tel['eon']} [{tel['era']}]", fontsize=11.5, fontweight='bold')
